...
class Message extends Base {
 ...
 async function slide(jid, title, msgText, m, footer, slides) {
    try {
        const cards = await Promise.all(slides.map(async ([
            imageUrl,
            slideTitle,
            bodyText,
            footerText,
            buttons // Array of buttons per slide
        ]) => {
            const mediaMessage = await prepareWAMessageMedia(
                { image: { url: imageUrl } },
                { upload: this.client.waUploadToServer }
            ...
            return {
                ...
                nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                    buttons: buttons.map(([buttonText, commandId, buttonType, buttonUrl]) => ({
                        name: buttonType,
...
function buildButtonParams(buttonType, textCommand, command, url) {
    switch (buttonType) {
        case "cta_url":
            return {
                display_text: textCommand,
